...
class ServiceAccountsMySqlDAO implements ServiceAccountsDAO{
	...
	public function insert($serviceAccount){
		$sql = 'INSERT INTO service_accounts (subdomain, database_name, database_user, date_registered, account_name, suffix, email, verified, trial_expiration_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery2($sql);
	...
	public function update($serviceAccount){
		$sql = 'UPDATE service_accounts SET subdomain = ?, database_name = ?, database_user = ?, date_registered = ?, account_name = ?, suffix = ?, email = ?, verified = ?, trial_expiration_date = ? WHERE id = ?';
		$sqlQuery = new SqlQuery2($sql);
	...
	protected function readRow($row){
		...
		$serviceAccount->trialExpirationDate = $row['trial_expiration_date'];
