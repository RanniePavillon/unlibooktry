...
<link href="/unlibooks/views/invoice/css/ajax.css" rel="stylesheet" type="text/css">
...
$allinvoice = DAOFactory::getTblAllInvoiceDAO()->queryByNewInvoiceId($invoiceid);

if (count($allinvoice) > 0) {
    Session::setSession('allinvoiceid', $allinvoice[0]->id);
    $total_amount_lines = $allinvoice[0]->totalAmountLines;
}

$payments = DAOFactory::getTblEnterPaymentDAO()->queryByAllInvoiceId($allinvoice[0]->id);
$balance = 0;
if (count($payments) > 0) {
...
<div class="invoiceHolderENTERPayment">
    <form method='post' action='<?php echo URL ?>invoice/collection' id='form' class="form1ENTERPayment">
        <div id="enterENTERPayment">
            ...
            <div>
                <div class="EpaymentText">Billing No:<input type="text" id="textIn2ENTERPayment" value="<?php echo $invoice_number ?>"></div>		
            </div>
            <ul>
            ...
            <ul>
                <div class="invoiceTAENTERPayment">Billing Total Amount: <input attribute readonly="readonly" type="text" class="invoiceTA2ENTERPayment" value="<?php echo $total_amount_lines ?>"></div>
            </ul>
            <ul>
