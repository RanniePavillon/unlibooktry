...
class ReportClass {
    ...
    static function incomeStatement($itpdate) {
        ...
        if ((isset($_POST['searchtype']) && $_POST['searchtype'] == 'monthly')) {
            ...
            $transdate = " Year(tbl_trial_balance_trans.date) = " . date('Y', strtotime($_POST['date']))
                    . " && Month(tbl_trial_balance_trans.date)=" . date('m', strtotime($_POST['date']));
...
//                    . " && Month(tbl_journal_entry.trans_date)=" . date('m', strtotime($_POST['date']));
        } elseif ((isset($_POST['searchtype']) && $_POST['searchtype'] == 'annual') || isset($_POST['year'])) {
            $date = $_POST['year'];
            $transdate = "Year(tbl_trial_balance_trans.date) = " . $_POST['year'];
//            $nidateissued = "Year(a.date_issued) = " . $_POST['year']; // . " GROUP BY MONTH(a.date_issued)";
//
//            $nedateissued = " Year(tbl_new_expense.date_issued) = " . $_POST['year']; //. " GROUP BY MONTH(tbl_new_expense.date_issued)";
//
//            $jetransdate = " Year(tbl_journal_entry.trans_date) = " . $_POST['year']; //. " GROUP BY MONTH(tbl_journal_entry.trans_date)";
        }
//        elseif ($itpdate != '') {
//            $nidateissued = " Year(a.date_issued) = " . date('Y', strtotime($itpdate))
//                    . " && Month(a.date_issued)=" . date('m', strtotime($itpdate));
//
//            $nedateissued = " Year(tbl_new_expense.date_issued) = " . date('Y', strtotime($itpdate))
//                    . " && Month(tbl_new_expense.date_issued)=" . date('m', strtotime($itpdate));
//
//            $jetransdate = " Year(tbl_journal_entry.trans_date) = " . date('Y', strtotime($itpdate))
//                    . " && Month(tbl_journal_entry.trans_date)=" . date('m', strtotime($itpdate));
//        }


        if (isset($_POST['date']) || isset($_POST['year']) || $itpdate != '') {
...
//            $txt = "select income.trans_date, sum(income.expense) as expense, sum(income.service_income) as service_income, sum(income.other_income) as other_income
...
//                            end as trans_date, tbl_expense_amount.sub_total_amount as expense, 'service_income', 'other_income'
...
//                    union all
//
//                      select tbl_journal_entry.trans_date,(tbl_journal_lines.debit - tbl_journal_lines.credit) as expense,'',''
...
//                            end as trans_date,
//                      '',(e.sub_total_amount - e.discount_amount) as service_income,''
//                      from tbl_new_invoice a
//                      inner join tbl_client c on a.client_id = c.id and c.org_id = " . $orgid . " and a.`status` in ('posted','reversed')
...
//                    union all 
//
//                      select tbl_journal_entry.trans_date,'','',(tbl_journal_lines.credit - tbl_journal_lines.debit) as other_income
//                      from tbl_journal_entry
//                      inner join tbl_journal_lines on tbl_journal_entry.id = tbl_journal_lines.journal_entry_id and tbl_journal_entry.org_id = " . $orgid . " 
//                      inner join tbl_coa on tbl_journal_lines.account_code = tbl_coa.id and tbl_coa.account_num like '%4001%'  
//                      WHERE " . $jetransdate . " 
//                      ) income
//
//                    group by month(income.trans_date)";
//echo $txt;
//exit;
...
//                    echo $item['PSI'] + $item['Other_Income']; //- $item['Sales_Disc'] - $item['Cost_of_Service'] - $item['Expenses'];exit;
    ...
    function provisionaryIncomeTax($net) {
...
//         print_r($form2550q);
    ...
    /*function balanceSheet() {
        ...
        if ($_POST['searchtype'] == 'monthly') {
            $date = date('F', strtotime($_POST['date'])) . ' - ' . date('Y', strtotime($_POST['date']));
            if (date('m', strtotime($_POST['date'])) > 1) {
                $cdate = " tbl_trial_balance_trans.date >= '" . date('Y', strtotime($_POST['date'])) . "-01-01' "
                        . "&& tbl_trial_balance_trans.date <='" . date('Y-m', strtotime($_POST['date'])) . "-31'";
            } else {
                $cdate = " Year(tbl_trial_balance_trans.date) = " . date('Y', strtotime($_POST['date']))
                        . " && Month(tbl_trial_balance_trans.date)=" . date('m', strtotime($_POST['date']));
            }

        } elseif ($_POST['searchtype'] == 'annual') {
            $date = $_POST['year'];
            $cdate = " Year(tbl_trial_balance_trans.date) = " . $_POST['year'];
        }
        ...
        $txt = "
                ...
                from (	
                                        select 	tbl_trial_balance_trans.date, tbl_coa.account_num,
                                                                ...
                                                                case when tbl_coa.account_num like '%2000-001%' then tbl_trial_balance.balance end as 'AP',
                                                                case when tbl_coa.account_num like '%2000-002%' then tbl_trial_balance.balance end as 'AE',
                                                                case when tbl_coa.account_num like '%2000-010%' then tbl_trial_balance.balance end as 'ITP',
                                                                case when tbl_coa.account_num like '%2001%' then tbl_trial_balance.balance end as 'LTB',
                                                                case when tbl_coa.account_num like '%2002%' then tbl_trial_balance.balance end as 'DC',
                                                                case when tbl_coa.account_num like '%2003%' then tbl_trial_balance.balance end as 'OL',
                                                                case when tbl_coa.account_num like '%3000%' then tbl_trial_balance.balance end as 'Capital',
                                                                case when tbl_coa.account_num like '%3001%' then tbl_trial_balance.balance end as 'NOIL',
                                                                case when tbl_coa.account_num like '%3002%' then tbl_trial_balance.balance end as 'Drawing',
                                                                case when tbl_coa.account_num like '%2000-003%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-004%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-005%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-006%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-007%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-008%' then tbl_trial_balance.balance 
                                                                                 when tbl_coa.account_num like '%2000-009%' then tbl_trial_balance.balance 
                                                                end as 'OCL'
...
//                $value->outputVat = $item['Output_Vat'];
        ...
        $value->totalcapital = $value->capital + $value->NOIL + $value->Drawing;
//			 print_r($value);
        return $value;
    }*/
