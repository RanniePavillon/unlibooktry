...
<div class="invoiceHolderExpense">
    <div id="news" style="<?= $getPayable->status == 'posted' ? 'float:left;width:100%;height:52px;' : '' ?>">
        ...
        <?php if ($getPayable->status == 'posted' || $getPayable->status == 'reversed') {
            ...
            <a href="<?= URL ?>expenses/newBillPayable">	 
                <label class="crossBillPayablePost"><input type="button" class="createAllpayable buttonslarge" value="RECORD BILL PAYABLE" style="padding-left:35px;"></label>
            </a>
        <?php
        }
    ...
    <form method="post" action="" class="form1NExpense" id="form">
        <div id="containerNExp" style="float:left;padding:0px!important;">
            <table class="tbleExpense">
                ...
                <tr class="cus">
                    ...
                    <td style="border:none;">
                        <input type="text" class="sc tinexp" style="margin-top:3px;" name="tin" id="tin" readonly>
                    </td>
                </tr>
                <tr class="cus">
                    <td class="addSample" style="border:none;"><div style="margin-top:-37px;">Address:</div></td>
                    <td style="border:none;">
                ...
                <tr>
                    <td style="padding-top:50px;border:none;" colspan="2">
...
?>				
        ...
        <div style="float:right;margin-right:25px;">			
            <table class="expensetableNewNew">		
                <tr class="cus">
                    ...
                    <td style="border:none;">
                        <input type="text" style="background:#c8c8c8;border:none;" class="expenseText" style="margin-top:10px;" name="payableNumber" required readonly
                ...
                <tr class="cus">
                    ...
                    <td style="border:none;"><input type="text" class="expenseText" name="refNum" 
                                                    value="<?= $this->getPayable->refNum ?>" maxlength="12"></td>
                </tr>
                ...
                <tr class="cus">
                    <td style="padding-top: 15px;border:none;"><div style="margin-top:-42px;">Remarks:</div></td>
        ...
        <table class="table1NewExpensesNew">
            ...
            <tr class="tablet">
                ...
                <th width= "" style="border:none;">Account Code</th>
...
<?php if (!$this->getPayableLine) { ?>
                    <tr class="no-bg borderbelownew">
                        ...
                        <td class="neg" style="border:none;">
                            <input type="text" class="acctCode" readonly="">
                        </td>
    ...
    foreach ($this->getPayableLine as $ln) {
        ...
        ?>
                            ...
                            <td class=""style="border:none;padding: 0px 1px 0px 1px;">
                                <select name="wht[]">
                                    <option value="0" <?= $ln->wht == 0 ? 'selected="selected"' : '' ?> >--SELECT--</option>
                                    <option value="0" <?= $ln->wht == 0 ? 'selected="selected"' : '' ?> >0%</option>
                                    <option value="1" <?= $ln->wht == 1 ? 'selected="selected"' : '' ?> >1%</option>
                                    <option value="2" <?= $ln->wht == 2 ? 'selected="selected"' : '' ?> >2%</option>
                                    <option value="5" <?= $ln->wht == 5 ? 'selected="selected"' : '' ?> >5%</option>
                                    <option value="10" <?= $ln->wht == 10 ? 'selected="selected"' : '' ?> >10%</option>
                                    <option value="15" <?= $ln->wht == 15 ? 'selected="selected"' : '' ?> >15%</option>
...
?>
                                            <!--<option value="<?php echo $coa->id ?>" <?php //echo ($coa->id == $this->getPayable->glPosting) ? 'selected' : '' ?>><?php echo $coa->accountNum ?> - <?php echo $coa->accontName ?></option>-->
...
//						}
//					}
                $array = array('2000-001-001', '2000-001-002', '2000-002', '2000-003', '2000-004',
                    '2003-001');
                $coas3 = TblCoaMySqlExtDAO::getUserCoaExpense(Session::getSession('orgid'), $array);

                if ($coas3) {
                    foreach ($coas3 as $coa) {
//                        if ($coa->activeAccount == 'yes') {
                            ?>
                            <option value="<?php echo $coa->id ?>" <?php echo ($coa->id == $this->getPayable->glPosting) ? 'selected' : '' ?>><?php echo $coa->accountNum ?> - <?php echo $coa->accontName ?></option>
                            <?php
...
?>
            ...
            <tr class="gtotal">
                <td style="padding:5px;border:none;font-size:12px;font-family:Verdana;" class="gt">Total Payable:</td>
                <td class="amountNewExpense " id="gt" style="border:none;">
                    <input type="text" class="isFloat grandTotal" name="grandTotal" readonly
                           value="<?= Controller::getFloat($this->getPayableAmount->grandTotal) ?>" style="background:none;color:#fff;font-weight:bold;font-size:12px;font-family:Verdana;"/>
                </td>
            </tr>
        </table>
        <div>
...
//        });
        $('#form').submit(function () {
            conSubmit = confirm("Are you sure you want to save this bill payable?");
            if (!conSubmit) {
                return false;
            }
            stat = false;
...
//            return false;
        ...
        $(document).on("change", ".selectNewExp", function () {
            ...
            value = $(this).val();
            code = $(this).find('option:selected').attr('accountNum');
            $(this).parent('td').parent('tr').find('.acctCode').val(code);
            allowWithHolding = false;
            $('.wtHolder').addClass('hidden');
            // $('select[name="ewt"]').val(0);
    ...
    ?>
            ...
            $('.reverseExpense').click(function () {
                ...
                if (confirmReverse) {
                    ...
                    $('.popBack').html('<div class="loadingHolder"><img src="' + URL + 'public/images/processing2.gif" class="loadingimg" /></div>');
                    $('.popBack').removeClass('hidden');
                    $.post(URL + 'expenses/setReverseBillPayable', {'id': '<?= $this->getPayable->id ?>', 'token':token})
                            .done(function (returnData) {
