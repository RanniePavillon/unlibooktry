...
@media print{
	#headerHead, #accpLiveChat, .mainbodyExpenses, .footerHolderNew, .invoiceHolderExpense, .voucher, .invoiceHolderCollection, .xButton{
		display:none;
	}
	.evHolder{
		zoom:118%;
...
<div class="evCont">
	<div class="evHolder expense_holder">
		...
		<div class="compInfo" style="float:left;text-align:center;margin-left:207px;margin-top:5px;">
			<input type="text" id="expense_id" class="hidden" value="">
			...
			<ul>
				<li style="font-size:22px;">
					<?php
					//echo ($payment->glPosting);
					if ($payment->glPosting != 3 ){
						echo 'CHECK VOUCHER';
					}
					else if($payment->glPosting == 3 ){
						echo 'CASH VOUCHER';
					}else{
		...
		<div class="rightInfo" style="float:right;margin-right:30px;">
			<ul>
				<li>Payable No.: <input type="text" class="inpEV" value="<?= $payment->colNum ?>" style="margin-left:9px;" readonly></li>
				<li>Date Issued: <input type="text" class="inpEV" style="letter-spacing:1;margin-left:7px;" value="<?= date('m/d/Y',strtotime($payment->dateReceived)) ?>" readonly></li>
				<li>Ref No.: 
					<input type="text" class="inpEV" style="margin-left:35px;" value="<?= $payment->refNum ?>" readonly>
				</li>
			</ul>
		</div>
		<div style="clear:both;"></div>
...
<script>
	$(function(){
		id = '<?= $payment->id ?>';$('#expense_id').val();
		$('.pdfButton').click(function(){
			window.open(URL + 'expenses/printPDFpayment?id='+id);
		});
