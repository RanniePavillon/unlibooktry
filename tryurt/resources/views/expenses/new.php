...
<div class="invoiceHolderExpense">
	...
	<div style="float:right;">
		<a href="<?php echo URL ?>expenses/add">	
			<label class="crossexpensePost"><input type="button" class="createAll buttonslarge" value="RECORD EXPENSES" style="padding-left:33px;"></label>
		</a>
	</div>
	<?php }
    ...
    <form method="post" action="" class="form1NExpense" id="form">
        <div id="containerNExp" style="float:left;padding:0px!important;">
            <table class="tbleExpense">
                ...
                <tr>
                    <td style="padding-top:30px;border:none;" colspan="2">
                ...
                <?php if ($info->typeOfTax == "percentage") {
                    ...
                    <tr>
                        <td colspan="2" style="border:none;padding-top:7px;"> 
                            <input type="checkbox" id="includeInputTax" name="includeInputTax" value="yes" style="-webkit-transform: scale(1.2);"
        ...
        <?php /* if ($this->getExpense->status == 'open' || $this->getExpense->status == '') { ?>
            <div class="addlinediv">
                <input type="button" name="size" value="ADD LINE" class="inputAddline" id="addtask" <?php //echo $invoicestat2    ?> value=""> 
            </div>
        <?php } */ ?>
    ...
    function computeAmount() {
        ...
        if ($('#includeInputTax').is(':checked')) {
            ...
            $('input[name="amount[]"]').each(function () {
				...
				discountAmount += (whtRate / 100) * (value / (1 + (rate / 100))); ($(this).parent('td').prev('td').find('select').val() / 100) * value;
            });
...
if ($this->getExpense->status == 'posted') {
    ?>
            ...
            $('.reverseExpense').click(function() {
                ...
                if (confirmReverse) {
                    $.post(URL + 'expenses/setReverse', {'id': '<?= $this->getExpense->id ?>', 'token':token})
                            .done(function(returnData) {
                                returnData = returnData.trim();
                                if (returnData == '') {
                                   location = URL + 'expenses';
