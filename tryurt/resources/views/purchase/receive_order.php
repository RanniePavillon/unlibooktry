...
<div class="mainbodyInvoice footerHeight">
    <form method='post' action='<?php echo URL ?>purchase/receiveOrder?page=<?= $page ?>&ipp=<?= $ipp ?>' id='form'>
        <div class="invoiceHolderInvoice">
            <div id="newInvoice" class="headings">
                <label class="headText">ALL RECEIVED ORDERS</label>
                <?php if ($useraccess->Add == 'yes') { ?>
                    <a href="<?php echo URL ?>purchase/newReceiveOrder">	
                        <!-- <label class="crossReceiveOrder"> -->
                            <input type="button" class="createNI buttonslarge" value="RECORD RECEIVE ORDER">
                        <!-- </label> -->
                    </a>
                <?php } ?>
            </div>	
        ...
        <div class="moduleHeight">
            ...
            <table class="cnitableAllInvoice">
                <tr class="cnitableAllInvoiceTr" id="hoverTr">
                    ...
                    <th style="text-align:left;padding:5px;padding-left: 5px;width:11%;"><a onclick="sort('invoice', '<?php echo ($sortdirection == 'desc') ? 'asc' : 'desc' ?>')">RO No.</a></th>
                    <th style="text-align:left;padding-left: 5px;width:20%;">Date</th>
                    <th style="text-align:left;padding-left: 5px;width:10%;">Vendor Name</th>
                    <th style="text-align:right;padding-right:0px;width:11%;">Remarks</th>
                    <th style="text-align:right;padding-right: 5px;width:12%;">Total Amount</th>
                ...
                if ($this->data) {
                    foreach ($this->data as $item) {
                        ...
                        <tr class="tablecni" id="hoverTr">
                            ...
                            <td style="text-align:left;padding-left: 5px;" class=""><a href="#" onclick="editrec(<?php echo $item->id ?>)"><?php echo $item->receivedOrderNo ?></a></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><?php echo date('m/d/Y', strtotime($item->dateIssued)) ?></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><?php echo $item->name ?></td>
                            <td style="text-align:right;padding-right: 5px;" class=""></td>
...
<script>
    invoiceprintview = '<?= Session::getSession('receiveorderid') ?>';

<?php 
Session::setSession('receiveorderid', '');
if (Session::getSession('status') == 'posted'): ?>
