...
$(function () {
    ...
    $('.printPrev').click(function () {
        if ($('.chk:checked').length > 1) {
            ...
            $.post(URL + 'purchasereturn/purchaseReturnPrev', {'id': $('.chk:checked').not('.hide').first().val()})
...
function editrec(returnid) {
    if ($('.chk').is(':checked') || returnid != '') {
        var checked = "";
        count = 0;
        myThis = '';
        $("input[name='chk[]']:checked").each(function ()
        ...
        if (status == 'Posted' || status == 'Reversed' && stat != 'copy') {
            alert('Purchase Return may be posted/reversed. You can no longer edit the Purchase Return.');
            $('input[name="chk[]"]:checked').prop('checked', false);
            return false;
        }

        $().redirect(URL + 'purchase/newPurchaseReturn', {'id': checked, 'status': stat, 'task101': 'updatepurchasereturn'})
...
function deleterec() {
    ...
    if ($('.chk').is(':checked')) {
        ...
        if (status == 'Posted' || status == 'Reversed') {
            alert('Purchase return(s) may be posted/reversed. You can no longer delete the purchase return.');
            $('input[name="chk[]"]').prop('checked', false);
            return false;
        }

        if (confirm('Are you sure you want to delete the following record(s)?')) {
            $('input[name="task"]').val('delpurchasereturn');
            $('#form').submit();
        }
    } else {
