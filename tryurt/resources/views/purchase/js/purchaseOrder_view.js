...
$(function () {
    ...
    $('.printPrev').click(function () {
        if ($('.chk:checked').length > 1) {
            ...
            $.post(URL + 'purchase/purchaseOrderPrev', {'id': $('.chk:checked').not('.hide').first().val()})
...
function editrec(invoiceid) {
    if ($('.chk').is(':checked') || invoiceid != '') {
        ...
        $().redirect(URL + 'purchase/newPurchaseOrder', {'id': checked, 'status': stat, 'task101': 'updatepurchaseorder'})
...
function deleterec() {
    ...
    if ($('.chk').is(':checked')) {
        ...
        if (confirm('Are you sure you want to delete the following record(s)?')) {
            $('input[name="task"]').val('delpurchaseorder');
            $('#form').submit();
        }
    } else {
