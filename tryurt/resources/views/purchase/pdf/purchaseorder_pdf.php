...
function data() {
    $_POST['invoiceid'] = $_GET['purchaseorderid'];

    $client = new TblSupplier();
    $invoice = new TblPurchaseOrder();
    $taskline = array();
    
    $invoiceAmount = new TblPurchaseOrderAmount();
    if (isset($_POST['invoiceid'])) {
        $invoice = DAOFactory::getTblPurchaseOrderDAO()->load($_POST['invoiceid']);
        $client = DAOFactory::getTblSupplierDAO()->load($invoice->supplierId);
        $taskline = DAOFactory::getTblPurchaseOrderLinesDAO()->queryByPurchaseOrderId($invoice->id);
//        $itemline = TblInvoiceLinesMySqlExtDAO::getItem($invoice->id);
//        return $_POST['invoiceid'];

       
        $invoiceAmt = DAOFactory::getTblPurchaseOrderAmountDAO()->queryByPurchaseOrderId($invoice->id);
...
//        $payment = DAOFactory::getTblPaymentTermsDAO()->load($client->termsId);
    ...
    $data = '
			<br/>
			<div id="newInvoice" style="text-align:center;border:1px solid #000;">PURCHASE ORDER</div>
			<br/>
			<br/>
			<table class="tbleInvoice" style="cellpadding:1px;" >
				<tr class="invoicetr">
					<td style="width:430px; font-size:12px; height:18px;">Vendor Name: ' . $client->name . '</td>
					<td style="height:18px;">PO No:</td>
					<td style="width:148px;">'.$invoice->purchaseOrderNo.'</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">
					...
					<td style="width:123px;height:18px;">PO Date:</td>
