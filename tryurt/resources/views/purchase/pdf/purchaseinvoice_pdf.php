...
function data() {
    $_POST['invoiceid'] = $_GET['purchaseinvoiceid'];

    $client = new TblSupplier();
    $invoice = new TblPurchaseInvoice();
    $taskline = array();
    
    $invoiceAmount = new TblPurchaseInvoiceAmount();
    if (isset($_POST['invoiceid'])) {
        $invoice = DAOFactory::getTblPurchaseInvoiceDAO()->load($_POST['invoiceid']);
        $client = DAOFactory::getTblSupplierDAO()->load($invoice->supplierId);
        $taskline = DAOFactory::getTblPurchaseInvoiceLinesDAO()->queryByPurchaseInvoiceId($invoice->id);
//        $itemline = TblInvoiceLinesMySqlExtDAO::getItem($invoice->id);
//        return $_POST['invoiceid'];

       
        $invoiceAmt = DAOFactory::getTblPurchaseInvoiceAmountDAO()->queryByPurchaseInvoiceId($invoice->id);
...
//        $payment = DAOFactory::getTblPaymentTermsDAO()->load($client->termsId);
    ...
    $data = '
			<br/>
			<div id="newInvoice" style="text-align:center;border:1px solid #000;">RECEIVING ORDER</div>
			<br/>
			<br/>
			<table class="tbleInvoice" style="cellpadding:1px;" >
				<tr class="invoicetr">
					<td style="width:430px; font-size:12px; height:18px;">Vendor Name: ' . $client->name . '</td>
					<td style="height:18px;">Receiving No:</td>
					<td style="width:148px;">'.$invoice->purchaseInvoiceNo.'</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">
					...
					<td style="width:123px;height:18px;">Received Order No:</td>
					<td style="">' . DAOFactory::getTblReceivedOrderDAO()->load($invoice->receivedOrderId)->receivedOrderNo . '</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">	
					...
					<td style="width:123px;height:18px;">Purchase Date:</td>
