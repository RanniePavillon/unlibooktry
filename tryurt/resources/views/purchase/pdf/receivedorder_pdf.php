...
function data() {
    $_POST['invoiceid'] = $_GET['receivedorderid'];

    $client = new TblSupplier();
    $invoice = new TblReceivedOrder();
    $taskline = array();
    
    $invoiceAmount = new TblReceivedOrderAmount();
    if (isset($_POST['invoiceid'])) {
        $invoice = DAOFactory::getTblReceivedOrderDAO()->load($_POST['invoiceid']);
        $client = DAOFactory::getTblSupplierDAO()->load($invoice->supplierId);
        $taskline = DAOFactory::getTblReceivedOrderLinesDAO()->queryByReceivedOrderId($invoice->id);
//        $itemline = TblInvoiceLinesMySqlExtDAO::getItem($invoice->id);
//        return $_POST['invoiceid'];

       
        $invoiceAmt = DAOFactory::getTblReceivedOrderAmountDAO()->queryByReceivedOrderId($invoice->id);
    ...
    $data = '
			...
			<table class="tbleInvoice" style="cellpadding:1px;" >
				<tr class="invoicetr">
					...
					<td style="width:148px;">'.$invoice->receivedOrderNo.'</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">
					...
					<td style="width:123px;height:18px;">PO No:</td>
					<td style="">' . DAOFactory::getTblPurchaseOrderDAO()->load($invoice->purchaseOrderId)->purchaseOrderNo . '</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">	
					...
					<td style="width:123px;height:18px;">Receiving Date:</td>
