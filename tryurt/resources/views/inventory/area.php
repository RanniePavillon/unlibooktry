...
<div class="mainbodyInvoice footerHeight">
    <form method='post' action='<?php echo URL ?>inventory/location?page=<?= $page ?>&ipp=<?= $ipp ?>' id='form'>
        <div class="invoiceHolderInvoice">
            <div id="newInvoice" class="headings">
                <label class="headText">ALL AREAS</label>
                <?php if ($useraccess->Add == 'yes') { ?>	
                    <a href="<?php echo URL ?>">	
                        <!-- <label class="crossSalesArea"> -->
                            <input type="button" class="createNLocation createArea buttonslarge" value="CREATE NEW AREA">
                        <!-- </label> -->
                    </a>
                <?php } ?>
            </div>	
                ...
                <div style="float:right;margin-right:30px;margin-top:4px;">
                    ...
                    <label class="labelclass">	
                        <select class="inumber" name="searchby" id="searchby">
                            <option class="invNoOption" value="1" <?php echo ($searchby == 1) ? 'selected' : '' ?>>AREA NO.</option>
                            <option class="invNoOption" value="2" <?php echo ($searchby == 2) ? 'selected' : '' ?>>AREA Name</option>
                        </select>
                    </label>
        ...
        <div class="moduleHeight">
            ...
            <table class="cnitableAllInvoice">
                <tr class="cnitableAllInvoiceTr" id="hoverTr">
                    ...
                    <th style="text-align:left;padding:5px;padding-left: 5px;width:11%;"><a onclick="sort('invoice', '<?php echo ($sortdirection == 'desc') ? 'asc' : 'desc' ?>')">Area No.</a></th>
                    <th style="text-align:left;padding-left: 5px;width:20%;">Area Name</th>
                    <th style="text-align:left;padding-left: 5px;width:10%;">Date Created</th>
                </tr>
                <?php
                if ($this->getArea) {
                    foreach ($this->getArea as $item) {
                        ?>
                        <tr class="tablecni" id="hoverTr">
                            <td style="text-align:center;" class=""><input type="checkbox" name='chk[]' class='chk' value="<?= $item->id ?>"></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><a href='#' onclick='editrec(<?= $item->id ?>)'><?= $item->areaCode ?></a></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><?= $item->areaName ?></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><?= $item->dateCreated ?></td>
                        </tr>
                        <?php
                    }
                }
