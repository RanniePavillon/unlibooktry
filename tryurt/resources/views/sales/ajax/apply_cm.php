    ...
    <form method="post" action="<?php echo URL ?>invoice/collection" class="cmForm boxshadow" id="form">
        ...
        <div class="formCm">
            <table class="formCmtable">
                ...
                if ($this->data2) {
                    foreach ($this->data2 as $cm) {
                        ?>
                        <tr>
                            <td><input type="<?php echo $_POST['type'] == 'sales' ? 'radio' : 'checkbox' ?>" value="<?php echo $cm->id ?>" class="chkcm" name="chkcm"/>
                                <input type="hidden" class="chk_cmamount" value="<?php echo $cm->grandTotal ?>" />
                                <input type="hidden" class="type" value="sr" />
                            </td>
                            <td><?php echo date('m/d/Y', strtotime($cm->dateIssued)) ?></td>
                            <td><?php echo $cm->type ?></td>
                            <td><?php echo $cm->salesReturnNo ?></td>
                            <td class="amountline"><?php echo Controller::getFloat($cm->grandTotal) ?></td>
                        </tr>
                        <?php
                    }
                }
