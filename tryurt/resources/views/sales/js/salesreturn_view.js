...
function editrec(invoiceid) {
    if ($('.chk').is(':checked') || invoiceid != '') {
        ...
        if (status == 'Posted' || status == 'Reversed' && stat != 'copy') {
            alert('Sales Return may be posted/reversed. You can no longer edit the Sales Return.');
            $('input[name="chk[]"]:checked').prop('checked', false);
            return false;
        }

        $().redirect(URL + 'sales/newSalesReturn', {'id': checked, 'status': stat, 'task101': 'updateinvoice'})
...
function deleterec() {
    ...
    if ($('.chk').is(':checked')) {
        ...
        if (status == 'Posted' || status == 'Reversed') {
            alert('Sales return(s) may be posted/reversed. You can no longer delete the sales return.');
            $('input[name="chk[]"]').prop('checked', false);
            return false;
        }

        if (confirm('Are you sure you want to delete the following record(s)?')) {
            $('input[name="task"]').val('delsalesreturn');
            $('#form').submit();
        }
    } else {
