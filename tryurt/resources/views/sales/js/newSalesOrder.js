$(function () {
    ...
    $('.outletCon').addClass('hidden');
    $('input[name="deductedTo"]').click(function () {
        if ($(this).val() == 'main inventory') {
            $('.outletCon').addClass('hidden');
        } else {
            $('.outletCon').removeClass('hidden');
        }
    });
    ...
    $('.form-Sales').submit(function () {
        ...
        $.ajaxSetup({async: false});
        $.post(URL + 'sales/clientCreditLimit', {client_id: $('#client').val(), amount: $('input[name="grandTotal"]').val()})
                .done(function (returnData) {
                    credit_limit = returnData;
                });

        if (credit_limit == 1) {
            alertWithoutNotice('Amount transaction is over the credit limit');
            $('.bPost').prop('disabled', false);
            $('input[name="status"]').val('');
            return false;
        }
        ...
        if ($('input[name="status"]').val() == 'post') {
            confirmation101 = confirm('Do you want to post the Sales Order?');
            if (confirmation101) {
                saveBilling();
                $('body').css('overflow', 'hidden');
            ...
            confirmation101 = confirm('Are you sure you want to save the Sales Order?');
            if (confirmation101) {
                saveBilling();
                $('body').css('overflow', 'hidden');
                $('.popBack').html('<div class="loadingHolder"><img src="' + URL + 'public/images/processing2.gif" class="loadingimg" /></div>');
    ...
    function saveBilling() {
        ...
        $.post(URL + 'sales/updateSalesOrder', $('.form-Sales').serialize())
                .done(function (returnData) {
                    $('body').css('overflow', 'hidden');

                    if (returnData == '') {
                        location = URL + 'sales/salesOrder';
                    } else if (returnData == 'add') {
                        location = URL + 'sales/newSalesOrder';
                    } else if (returnData == 'duplicate code') {
                        alertWithoutNotice('Sales order number already exists');
