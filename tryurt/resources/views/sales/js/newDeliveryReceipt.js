$(function () {
    ...
    $('.form-Sales').submit(function () {
        ...
        if ($('input[name="status"]').val() == 'post') {
            confirmation101 = confirm('Do you want to post the Delivery Receipt?');
            if (confirmation101) {
                saveBilling();
            ...
            confirmation101 = confirm('Are you sure you want to save the Delivery Receipt?');
            if (confirmation101) {
                saveBilling();
                $('body').css('overflow', 'hidden');
                $('.popBack').html('<div class="loadingHolder"><img src="' + URL + 'public/images/processing2.gif" class="loadingimg" /></div>');
...
function saveBilling() {
    ...
    $.post(URL + 'deliveryreceipt/updateDeliveryReceipt', $('.form-Sales').serialize())
            .done(function (returnData) {
                $('body').css('overflow', 'hidden');

                if (returnData == '') {
                    location = URL + 'sales/deliveryReceipt';
                } else if (returnData == 'add') {
                    location = URL + 'sales/newDeliveryReceipt';
                } else if (returnData == 'duplicate code') {
                    alertWithoutNotice('Delivery Receipt number already exists');
...
function showSr() {
    if ($('#client :selected').val() != '') {
        $.post(URL + 'sales/deliveryReceiptForm', {view: 'invoice', clientid: $('#client :selected').val()})
                .done(function (returnData) {
