...
<div class="mainbodyInvoice">
    <div id="new2s" style="margin-left:10px !important; padding-top:15px;">
        <label class="headTextNewInvoice headings">
			<?php
				if ($taskstat == 'updatesalesinvoice') {
					echo 'UPDATE';
				} elseif ($taskstat == 'reversedsalesinvoice') {
					echo 'REVERSED';
				} elseif ($taskstat == 'postedsalesinvoice') {
					echo 'POSTED';
				} else {
					echo 'CREATE NEW';
				}
        ...
        <div class="headertables">
            <table style="float:left;" class="table1header">
                            ...
                            if (count($customer) > 0) {
                                foreach ($customer as $item) {
                                    if ($item->active == 'yes' || $item->id == $invoice->clientId) {
                                        if (TblDeliveryReceiptMySqlExtDAO::getDeliveryReceiptByClient($item->id) || $invoice->id != '') {
                                            ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php
                        $deliveryreceipt = $invoice->clientId != '' ? DAOFactory::getTblDeliveryReceiptDAO()->load($invoice->deliveryReceiptId) : array();
                        ?>
                        <select name="deliveryReceiptId" id="deliveryReceiptId" class="sc deliverySelect">
                            <option value=""></option>
                            <?php
                            if ($deliveryreceipt) {
//                                foreach ($deliveryreceipt as $item) {
                                ?>
                                <option value="<?php echo $deliveryreceipt->id ?>" <?php echo ($deliveryreceipt->id == $invoice->deliveryReceiptId) ? 'selected' : '' ?>><?php echo $deliveryreceipt->deliveryReceiptNo; //. ';' . $item->dateIssued . ';' . $item->grandTotal   ?></option>
                                <?php
//                                }
                            }
                            ?>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="hidden" name="averageCost[]" value="<?php echo $salesorderline->averageCost ?>" />
        ...
        <div class="">
            ...
            <?php // foreach($driver as $each):?>
                                         <option><?php //echo $each->driverName  ?></option>
            <?php // endforeach;?>
                 </select> 
             </div>     -->
        </div>
        ...
        <div class="cm_content2">
            ...
            if ($invoice->cmId != 0) {
                ...
                $cm_amount = TblNewCreditMemoMySqlExtDAO::getTotalCm($invoice->cmId);
                ?>
            <input type="hidden" name="cmid" value="<?php echo $invoice->cmId ?>"  />
...
<?php if (Session::getSession('status') == 'posted'): ?>
    <?php Session::setSession('status', false); ?>
        printPreview('<?= Session::getSession('salesinvoice_id') ?>');
        $('input[type="text"], select, input[type="number"], input[type="date"], textarea, input[type="radio"]').prop('disabled', true);
<?php endif; ?>
<?php Session::setSession('salesinvoice_id', false); ?>
    ...
    $('#reverseNewInvoice').click(function () {
        ...
        if (confirm('Are you sure you want to reverse the Sales Invoice?')) {
            ...
            $.post(URL + 'salesinvoice/updateSalesInvoice', {'id': '<?= $invoice->id ?>', 'status': 'reverse', 'token':token})
