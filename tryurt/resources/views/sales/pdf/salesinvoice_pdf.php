...
function data() {
    $_POST['invoiceid'] = $_GET['salesinvoiceid'];

    $client = new TblClient();
    $invoice = new TblSalesInvoice();
    $taskline = array();

    $invoiceAmount = new TblSalesInvoiceAmount();
    if (isset($_POST['invoiceid'])) {
        $invoice = DAOFactory::getTblSalesInvoiceDAO()->load($_POST['invoiceid']);
        $client = DAOFactory::getTblClientDAO()->load($invoice->clientId);
        $taskline = DAOFactory::getTblSalesInvoiceLinesDAO()->queryBySalesInvoiceId($invoice->id);
//        $itemline = TblInvoiceLinesMySqlExtDAO::getItem($invoice->id);
//        return $_POST['invoiceid'];


        $invoiceAmt = DAOFactory::getTblSalesInvoiceAmountDAO()->queryBySalesInvoiceId($invoice->id);
    ...
    $data = '<br>
			<div id="newInvoice" style="text-align:center;border:1px solid #000;">SALES INVOICE</div>
			<br/>
			<table class="tbleInvoice" style="cellpadding:1px;" >
				...
				<tr class="invoicetr">
					...
					<td style="width:148px;"> '. DAOFactory::getTblDeliveryReceiptDAO()->load($invoice->deliveryReceiptId)->deliveryReceiptNo .' </td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr class="invoicetr">
					...
					<td style="height:18px;">SI No.:</td>
					<td style="width:148px;"> '. $invoice->salesInvoiceNo. '</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
    ...
    if (count($taskline) > 0) {
        $data .= '<div style="margin-top:130px;" height=12px></div>' .
                ...
                ' <table class="tblInvoice2" style="border:solid 1px black;" cellpadding="4">
                        <thead>
                            <tr class="newTblInv">
                                <th style="width:20%;border:solid 1px black;font-family:Tahoma;">Item Description</th>
                                <th style="width:7%;border:solid 1px black;text-align:center;">Unit</th>
                                <th style="width:30%;border:solid 1px black;text-align:center;">Quantity</th>
        ...
        foreach ($taskline as $i => $task) {
            ...
            $data .='           <tr >' .
                    '<td style="width:20%; height:20px; padding:5px;text-align:left;">' . $task->itemDescription . '</td>' .
                    '<td style="width:7%; height:20px; padding:5px;text-align:center;">' . $uom->abbreviation . '</td>' .
                    '<td style="width:30%; height:20px; padding:5px;text-align:center;">' . $task->qty . '</td>' .
    ...
    $data .= '<br/>' .
            ...
            '<table cellpadding="4";>' .
            '<tr>' .
            '<td class="" style="font-size:12px;height:18px;width:340px; border:none;" cellpadding="4";></td>' .
            ...
            '</tr>' .
			 '<tr>' .
            '<td class="" style="font-size:12px;height:18px;width:340px; border:none;" cellpadding="4";></td>' .
            '<td class="SAInvoice" style="font-size:12px;height:18px;width:130px; border-top:solid 1px black;border-left:solid 1px black;padding:5px;" cellpadding="4";>&nbsp; Net Amount:</td>' .
            '<td style="font-size:12px;height:18px;text-align:right; width:160px; border:none; border-top:solid 1px black;" cellpadding="4">' . number_format((float) $subtotal, 2) . '</td>' .
            '<td class="" style="font-size:12px;height:18px;width:7px; border-right:solid 1px black;border-top:solid 1px black;" cellpadding="4";></td>' .
            '</tr>' .
            '<tr class="trTotalInvoices">' .
            ...
            '<td class="" style="font-size:12px;height:18px;cellpadding="4";border:none;" ></td>' .
            '<td class="SAInvoice3" cellpadding="4" style="border-left:solid 1px black;border-bottom:solid 1px black;background-color: #C8C8C8;padding-left:5px;" cellpadding="4">&nbsp; Total Amount:</td>' .
    ...
    $data .='
			<div style="font-weight:bold;font-size:12px;text-align:center;">"THIS DOCUMENT IS NOT VALID FOR CLAIM OF INPUT TAXES"</div>
			<br/>
			<table><tr cellpadding="4">' .
            '<td colspan="4" style="
					padding:40px 10px;
					font-size:12px;
					width:400px;
            ...
            '</tr>' .
            '<tr>' .
            '<td style="font-size:11px;font-weight:bold;" width="400"></td>' .
