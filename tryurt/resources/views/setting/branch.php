    ...
    <form method='post' action='<?php echo URL ?>sales/items?page=<?= $page ?>&ipp=<?= $ipp ?>' id='form'>
        <div class="invoiceHolderInvoice">
            <div id="newInvoice" class="headings">
                <label class="headText">ALL OUTLET</label>
                <?php if ($useraccess->Add == 'yes') { ?>	
                    <a href="<?php echo URL ?>">	
                        <!-- <label class="crossSalesSettings"> -->
                            <input type="button" class="createNIItem buttonslarge newOutlet" value="CREATE NEW OUTLET">
                        <!-- </label> -->
                    </a>
                <?php } ?>
            </div>	
            ...
            <div class="center2">
                <div id="search3" style="float:left;margin-top:15px;">
                    <?php if ($useraccess->Edit == 'yes') { ?>
                        <input type="button" name="edit" value="EDIT" class="delete1 buttonHover buttoninvoices" id="edit" onclick="editrec('')" style="margin-left: 15px;">
                    <?php } //if ($useraccess->Add == 'yes') {  ?>
                    <input type="button" name="inactive" value="INACTIVE" class="copyNew buttonHover buttoninvoices hidden" onclick="inactive('', 'copy')">
                    <?php // }  ?>
                    <input type="button" name="sendinvoice" value="SEND INVOICE" class="sendI hidden buttoninvoices">
                    <input type="button" name="print" value="PRINT PREVIEW" class="printPrev buttonHover buttoninvoices hidden">
                    <input type="button" name="sendEmail" value="SEND EMAIL" class="sendEmail buttonHover hidden buttoninvoices hidden">
                </div>
                ...
                <div style="float:right;margin-right:30px;margin-top:4px;">
                    ...
                    <label class="labelclass">	
                        <select class="inumber" name="searchby" id="searchby">
                            <option class="invNoOption" value="1" <?php echo ($searchby == 1) ? 'selected' : '' ?>>OUTLET NAME</option>
                        </select>
                    </label>
        ...
        <div class="moduleHeight"class="moduleHeight">

            <table class="cnitableAllInvoice">
                <tr class="cnitableAllInvoiceTr" id="hoverTr">
                    <th style="text-align:center; width:3%"></th>
                    <th style="text-align:left;padding:5px;padding-left: 5px;">Outlet Code</th>
                    <th style="text-align:left;">Outlet Name</th>
                    <th style="text-align:left;padding-left: 5px;width:20%;">Email-Address</th>
                </tr>
                ...
                if ($this->data) {
                    foreach ($this->data as $item) {
                        ...
                        <tr class="tablecni" id="hoverTr">
                            <td><input type="checkbox" class="chk" value="<?php echo $item->id ?>" /></td>
                            <td><a href="#" onclick="editrec('<?php echo $item->id ?>')"><?php echo $item->outletCode ?></a></td>
                            <td style="text-align:left;" class=""><?php echo $item->outletName ?></td>
                            <td style="text-align:left;padding-left: 5px;" class=""><?php echo $item->emailAddress ?></td>
                        </tr>
                        <?php
                    }
                }
...
<script>
    $(function () {
        $('.newOutlet').click(function () {
            $.post(URL + 'setting/addBranch')
