...
<form method='post' action='<?php echo URL ?>report/new_salesitem' id='form'>
	<div class="mainformitemsum">
		<div class="sbis reportTitle">Purchase per Item</div>
		<div class="buttonFS">
		...
		<div class="DFstask">
			<div style="float:left;">
				<div class="reportcolor">
					Vendor: 
					<select name="vendor" class="itemSelect">
						<option value="%%">All</option>
						<?php
						foreach($report as $each){
						?>
							<option value="<?= $each['supplier_id']?>">
								<?= $each['supplier_name']?>
							</option>	
						<?php
						}
...
<div class="reportHolderContainer trhidden">
	...
	?>
		<div class="noSales">
			You don't have Purchase per Item.
		</div>
	<?php
	}
	?>
...
<script>
	...
	$('form').submit(function() {
		...
		// $('.reportHolderContainer').html(loading);
			$.post(URL + 'report/genPurchasePerItem', 
				{
	...
	$('.exportSalesTask').click(function(){
		location = URL + 'report/exportPurchasePerItem?from='+$('input[name="startdate"]').val()+'&to='+$('input[name="enddate"]').val() + '&vendor=' + $('select[name="vendor"]').val();
	});
	...
	/* viewMode codes */
		<?php
		 if (isset($_GET['startdate'])) {
			?>
				...
				$('select[name="vendor"').val('<?= $_GET['vendor'] ?>');
				$('.FSGentask').click();
		...
		?>
		 $('.viewMode').click(function() {
			...
			window.open(URL + 'report/newtab_PurchasePerItem?startdate=' + startdate + '&enddate=' + enddate + '&vendor=' + vendor);
