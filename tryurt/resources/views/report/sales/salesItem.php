...
<form method='post' action='<?php echo URL ?>report/new_salesitem' id='form'>
	<div class="mainformitemsum">
		<div class="sbis reportTitle">Sales per Item</div>
		<div class="buttonFS">
			...
			<div style="float:left;">
				<div class="reportcolor">
					...
					<?php
						if($report){
					?>
								<select name="itemId" class="itemSelect">
									<option value="%%">All</option>
					<?php

							foreach($report as $each){
					?>
									<option value="<?= $each['item_id']?>">
										<?= $each['description']?>
									</option>
							<?php
							}
...
<script>
	...
	$('form').submit(function() {
		...
		// $('.reportHolderContainer').html(loading);
			$.post(URL + 'report/genSalesItem', 
				{
					startdate: $('input[name="startdate"]').val(), 
					enddate: $('input[name="enddate"]').val(),
					itemId: $('select[name="itemId"]').val()
				}
				)
	...
	$('.exportSalesTask').click(function(){
		location = URL + 'report/exportSalesItem?startdate='+$('input[name="startdate"]').val()+'&enddate='+$('input[name="enddate"]').val()+'&itemId='+$('select[name="itemId"]').val();
	});
	...
	/* viewMode codes */
		<?php
		 if (isset($_GET['startdate'])) {
			?>
				...
				$('input[name="itemId"]').val('<?= $_GET['itemId'] ?>');
				$('.FSGentask').click();
		...
		?>
		 $('.viewMode').click(function() {
			...
			window.open(URL + 'report/newtab_salesItem?startdate=' + startdate + '&enddate=' + enddate + '&itemId=' + itemId);
